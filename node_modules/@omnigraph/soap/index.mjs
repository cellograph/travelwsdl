...
import { RegularExpression, GraphQLVoid, GraphQLURL, GraphQLByte, GraphQLDate, GraphQLDateTime, GraphQLDuration, GraphQLHexadecimal, GraphQLBigInt, GraphQLUnsignedInt, GraphQLTime } from 'graphql-scalars';
import { GraphQLDirective, DirectiveLocation, GraphQLString, GraphQLBoolean, GraphQLFloat, GraphQLInt, buildASTSchema, execute, visit, isNonNullType, isListType } from 'graphql';
...
class SOAPLoader {
    ...
    async fetchXSD(location, parentAliasMap = new Map()) {
        ...
        const xsdObj = parse(xsdText, PARSE_XML_OPTIONS);
    ...
    async loadWSDL(wsdlText) {
        ...
        const wsdlObject = parse(wsdlText, PARSE_XML_OPTIONS);
...
function createRootValueMethod(soapAnnotations, fetchFn) {
    const jsonToXMLConverter = new j2xParser({
        attributeNamePrefix: '',
        attrNodeName: 'attributes',
        textNodeName: 'innerText',
    });
    return async function rootValueMethod(args, context, info) {
        ...
        const responseJSON = parse(responseXML, PARSE_XML_OPTIONS);
...
export { SOAPLoader, createExecutorFromSchemaAST };
