...
const soapDirective = new graphql.GraphQLDirective({
    name: 'soap',
...
class SOAPLoader {
    ...
    loadXMLSchemaNamespace() {
        ...
        const simpleTypeGraphQLScalarMap = new Map([
            ['anyType', graphqlCompose.GraphQLJSON],
            ['anyURI', graphqlScalars.GraphQLURL],
            ['base64Binary', graphqlScalars.GraphQLByte],
            ['byte', graphqlScalars.GraphQLByte],
            ...
            ['duration', graphqlScalars.GraphQLDuration],
            ['float', graphql.GraphQLFloat],
            ['int', graphql.GraphQLInt],
            ['hexBinary', graphqlScalars.GraphQLHexadecimal],
            ['long', graphqlScalars.GraphQLBigInt],
            ['gDay', graphql.GraphQLString],
            ['gMonth', graphql.GraphQLString],
            ['gMonthDay', graphql.GraphQLString],
            ['gYear', graphql.GraphQLString],
            ['gYearMonth', graphql.GraphQLString],
            ['NOTATION', graphql.GraphQLString],
            ['QName', graphql.GraphQLString],
            ['short', graphql.GraphQLInt],
            ['string', graphql.GraphQLString],
            ['unsignedByte', graphqlScalars.GraphQLByte],
            ['unsignedInt', graphqlScalars.GraphQLUnsignedInt],
            ['unsignedLong', graphqlScalars.GraphQLBigInt],
            ['unsignedShort', graphqlScalars.GraphQLUnsignedInt],
    ...
    async loadSchema(schemaObj, parentAliasMap = new Map()) {
        ...
        if (!typePrefix) {
            typePrefix =
                schemaObj.attributes.id || ((_a = [...aliasMap.entries()].find(([, namespace]) => namespace === schemaNamespace)) === null || _a === void 0 ? void 0 : _a[0]);
            ...
            for (const serviceObj of definition.service) {
                ...
                for (const portObj of serviceObj.port) {
                    ...
                    for (const operationObj of portTypeObj.operation) {
                        ...
                        const operationFieldName = utils.sanitizeNameForGraphQL(`${typePrefix}_${serviceName}_${portName}_${operationName}`);
                        ...
                        for (const part of inputMessageObj.part) {
                            if (part.attributes.element) {
                                ...
                                rootTC.addFieldArgs(operationFieldName, {
                                    [elementName]: {
                                        type: () => {
                                            const elementNamespace = aliasMap.get(elementNamespaceAlias) || part.attributes[elementNamespaceAlias];
                                            if (!elementNamespace) {
                                    ...
                                    [partName]: {
                                        type: () => {
                                            ...
                                            if ('getFields' in inputTC && Object.keys(inputTC.getFields()).length === 0) {
                                                return graphqlCompose.GraphQLJSON;
                                            }
                                            return inputTC;
                                        },
                                        defaultValue: '',
    ...
    async fetchXSD(location, parentAliasMap = new Map()) {
        ...
        const xsdObj = fastXmlParser.parse(xsdText, PARSE_XML_OPTIONS);
    ...
    async loadWSDL(wsdlText) {
        ...
        const wsdlObject = fastXmlParser.parse(wsdlText, PARSE_XML_OPTIONS);
        ...
        if (!simpleTypeTC) {
            ...
            if (restrictionObj.attributes.base === 'string' && restrictionObj.enumeration) {
                ...
                for (const enumerationObj of restrictionObj.enumeration) {
                    ...
                    const enumKey = utils.sanitizeNameForGraphQL(enumValue);
                    values[enumKey] = {
                        value: enumValue,
                    };
                }
                ...
                simpleTypeTC = this.schemaComposer.createScalarTC(new graphqlScalars.RegularExpression(scalarTypeName, new RegExp(pattern)));
            }
            else {
    ...
    getInputTypeForComplexType(complexType, complexTypeNamespace) {
        ...
        if (!complexTypeTC) {
            ...
            if (Object.keys(fieldMap).length === 0) {
                complexTypeTC = graphqlCompose.GraphQLJSON;
            }
            else {
        ...
        if (!complexTypeTC) {
            ...
            if (complexType.complexContent) {
                for (const complexContentObj of complexType.complexContent) {
                    for (const extensionObj of complexContentObj.extension) {
                        const [baseTypeNamespaceAlias, baseTypeName] = extensionObj.attributes.base.split(':');
                        const baseTypeNamespace = aliasMap.get(baseTypeNamespaceAlias) || extensionObj.attributes[baseTypeNamespaceAlias];
                        if (!baseTypeNamespace) {
                            throw new Error(`Namespace alias: ${baseTypeNamespaceAlias} is undefined!`);
                        }
            ...
            if (Object.keys(fieldMap).length === 0) {
                complexTypeTC = this.schemaComposer.createScalarTC(graphqlCompose.GraphQLJSON);
            }
            else {
    ...
    buildSchema() {
        if (this.schemaComposer.Query.getFieldNames().length === 0) {
            this.schemaComposer.Query.addFields({
                placeholder: {
                    type: graphqlScalars.GraphQLVoid,
                },
            });
        }
...
function createRootValueMethod(soapAnnotations, fetchFn) {
    const jsonToXMLConverter = new fastXmlParser.j2xParser({
        attributeNamePrefix: '',
        attrNodeName: 'attributes',
        textNodeName: 'innerText',
    });
    return async function rootValueMethod(args, context, info) {
        ...
        const response = await fetchFn(soapAnnotations.baseUrl, {
            method: 'POST',
            body: requestXML,
            headers: {
        ...
        const responseJSON = fastXmlParser.parse(responseXML, PARSE_XML_OPTIONS);
        return normalizeResult(responseJSON.Envelope[0].Body[0][soapAnnotations.elementName]);
    };
}
function createRootValue(schemaAST, fetchFn) {
    const rootValue = {};
...
function createExecutorFromSchemaAST(schemaAST, fetchFn) {
